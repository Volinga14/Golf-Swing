...
function bindEvents() {
  ...
  els.ballResult.addEventListener("change", () => {
    ...
    updateAnalysis();
    if (state.ballPathAuto) suggestBallPath();
  });
  ...
  els.autoBallPathBtn.addEventListener("click", () => suggestBallPath());
  ...
  els.ballStage.addEventListener("click", (event) => {
    ...
    if (state.ballPath.length > 5) state.ballPath.shift();
    renderBall();
  });
...
async function runAutoAnalysis() {
  ...
  try {
    const analysis = await analyzeVideo(els.video, {
      fps: state.fps,
      onProgress: (progress) => {
    ...
    updateAnalysis();
    renderBall();
    els.analysisStatus.textContent = `Fases detectadas automáticamente. Señal de movimiento: ${analysis.summary.signal}/100 aprox.`;
...
function renderEvents() {
  els.eventGrid.querySelectorAll("[data-event-card]").forEach((card) => {
    ...
    small.textContent = Number.isFinite(frame)
      ? `${meta?.source === "manual" ? "Manual" : "Auto"} · ${meta?.confidence ?? 0}%`
...
function renderCards(container, items = [], emptyText) {
  ...
  items.forEach((item) => {
    ...
    article.innerHTML = `<header><strong>${title}</strong><span>${item.nextMetric || ""}</span></header><p>${body}</p>`;
...
function suggestBallPath() {
  ...
  state.ballPath = [
    { x: 0.52, y: 0.72 },
    { x: 0.58 + curve.start, y: 0.52 },
    { x: 0.66 + curve.mid, y: 0.34 },
    { x: 0.78 + curve.end, y: 0.22 }
  ].map((point) => ({ x: clamp(point.x, 0.04, 0.96), y: clamp(point.y, 0.04, 0.92) }));
...
function renderBall() {
  resizeBallCanvas();
  const label = state.ballPathAuto ? "Sugerido" : state.ballPath.length ? "Manual" : "Sin camino";
  els.ballPathLabel.textContent = label;
  els.ballPathSummary.textContent = state.ballPath.length
    ? `${state.ballPath.length} puntos marcados. Resultado: ${ballResultLabel(state.ballResult)}.`
    : "Sube un vídeo y usa “Sugerir camino” o marca puntos manuales.";
